<!-- PREMIUM Plan -->
<div class="col-lg-6">
    <div class="block block-bordered block-link-hover3 text-center" href="javascript:void(0)">
        <div class="block-header" style='padding-top: 10px; 
             padding-left: 10px; padding-right: 10px; padding-bottom: 5px;
             border-bottom: none;'>
            <div style='padding:5px; background-color:rgb(30, 115, 190); height: 274px;'>
                <h3 class="push-20-t block-title h1 font-w700 text-white" 
                    style="font-family: 'Montserrat', sans-serif; font-size: 25px;">Business</h3>
                <div class="push-10-t block-title h6 text-white" 
                     style="font-size: 14px; text-transform: none; font-weight: 100;
                     font-style: italic; font-family: 'Lora', serif;">&nbsp;</div>
                <div class="text-white" 
                     style="font-family: 'Montserrat', sans-serif; 
                     font-style: normal; font-weight: normal; height: 200px;">
                    <span style="font-size: 30px; top: -20px; position:relative;">$</span>
                    <span style="font-size: 60px; top: -20px; padding-left: 10px;">134/mth</span>
                    <div class="push-30-t block-title h6 text-white" 
                         style="font-size: 14px; text-transform: none; font-weight: 100;
                         font-style: italic; font-family: 'Lora', serif;">For Power Users</div>
                </div>
            </div>
        </div>
        <div class="block-content" style='padding-top: 0px;
             padding-left: 10px; padding-right: 10px; padding-bottom: 10px;'>
            <div>
                <div style="font-family: 'Raleway'; padding-top: 35px; font-weight: 700; 
                     padding-bottom: 20px; padding-left: 30px; padding-right: 30px;">
                    <p style="border-bottom: 1px dotted #bdbdbd; line-height: 50px; color: rgb(0, 0, 0); text-align:center; font-size: 14px;">
                        Inclusive of Premium Subscription
                    </p>
                    <p style="border-bottom: 1px dotted #bdbdbd; line-height: 50px; color: rgb(0, 0, 0); text-align:center; font-size: 14px;">
                        5 more Instagram Accounts with all the Premium functions!
                    </p>
                    <p style="border-bottom: 1px dotted #bdbdbd; line-height: 50px; color: rgb(0, 0, 0); text-align:center; font-size: 14px;">
                        Gain access to our Auto-Engagement community
                    </p>
                    <p style="border-bottom: 1px dotted #bdbdbd; line-height: 50px; color: rgb(0, 0, 0); text-align:center; font-size: 14px;">
                        Send follow up direct messages automatically
                    </p>
                    <p style="border-bottom: 1px dotted #bdbdbd; line-height: 50px; color: rgb(0, 0, 0); text-align:center; font-size: 14px;">
                        EXCLUSIVE access to our full suite of Business Advance training videos
                    </p>
                </div>
                <div style='padding-top: 0px; padding-bottom: 40px;'>
                    <a id="submit-button" href='#' class="text-white" style='
                       background-color:rgb(30, 115, 190); 
                       padding: 10px 50px;
                       font-weight: 600; 
                       font-size: 15px; 
                       border: solid 1px #D3D3D3;
                       text-align: center;
                       text-transform: uppercase;'>
                        Upgrade
                    </a>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- END PREMIUM Plan -->