...
class InteractionFollow extends Command
{
					...
					foreach ($instagram_profiles as $ig_profile) {
						...
						if ($ig_profile->auto_follow_ban == 1) {
							if ($ig_profile->auto_follow_ban_time !== NULL && $ig_profile->auto_follow_ban_time >= \Carbon\Carbon::now()) {
								...
								if ($ig_profile->auto_follow_ban_time === NULL) {
						...
						if ($ig_profile->next_follow_time === NULL) {
							...
							if (\Carbon\Carbon::now()->gte(new \Carbon\Carbon($ig_profile->next_follow_time))) {
								dispatch((new \App\Jobs\InteractionFollow(\App\InstagramProfile::find($ig_profile->id)))
									->onQueue('follows'));
								$this->line("[Follow Interactions] queued " . $ig_profile->insta_username);
							}
	...
	private function jobHandle($ig_profile)
	{
		...
		if (($auto_unfollow == 1 && $auto_follow == 0) || ($auto_follow == 1 && $auto_unfollow == 1 && $unfollow == 1)) {
			...
			if (($unfollow == 0 && $auto_follow == 1) || ($auto_follow == 1 && $auto_unfollow == 0)) {
	...
	private function autoFollow($ig_profile)
	{
		...
		if ($use_hashtags == 1 && count($target_hashtags) > 0) {
					...
					foreach ($hashtag_feed->items as $item) {
						...
						if (InstagramProfileFollowLog::where('insta_username', $insta_username)->where('follower_id', $user_to_follow->pk)->count() > 0) {
							...
							if ($user_to_follow->is_private) {
								...
								if ($user_to_follow->has_anonymous_profile_picture) {
									...
									try {
										...
										if (stripos(trim($request_ex->getMessage()), "feedback_required") !== FALSE) {
											...
											if (stripos(trim($request_ex->getMessage()), "Feedback") !== FALSE) {
												$ig_profile->feedback_required = 1;
			...
			if ($use_hashtags == 0 && count($target_usernames) > 0) {
										...
										try {
											...
											if ($follow_resp->friendship_status->following == TRUE) {
												...
												if ($follow_resp->friendship_status->is_private) {
													...
													if ($follow_resp->friendship_status->following == FALSE) {
				...
				if ($use_hashtags == 0 && count($target_usernames) == 0 && count($target_hashtags) == 0) {
					$throttle_count = 0;
					try {
						if ($this->niche == 0) {
							exit();
