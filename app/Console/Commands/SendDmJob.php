...
class SendDmJob extends Command
{
	...
	public function handle()
	{
		...
		$execute_script = 1;
		$execute_flags  = DB::select("SELECT * FROM morfix_settings WHERE setting = 'dm_offset' AND value = ?;", [ $offset ]);
		foreach ($execute_flags as $execute_flag) {
			$execute_script = 0;
		}

		if ($execute_script == 1) {
			DB::insert("INSERT INTO morfix_settings (setting, value) VALUES (?,?);", [ 'dm_offset', $offset ]);
		} else {
			exit();
		}

		if (NULL !== $this->argument("email")) {
			$users = DB::select("SELECT * FROM insta_affiliate.user u WHERE u.email = ?;", [ $this->argument("email") ]);
		} else {
		...
		foreach ($users as $user) {
			...
			$instagram_profiles = DB::select("SELECT id, insta_username, "
			...
			foreach ($instagram_profiles as $ig_profile) {
				...
				$dm_jobs = DB::select("SELECT job_id, recipient_username, recipient_insta_id, message, follow_up_order "
				...
				if ($ig_profile->proxy === NULL) {
					$proxies = DB::select("SELECT proxy, assigned FROM insta_affiliate.proxy WHERE assigned = 0 LIMIT 2;");
					foreach ($proxies as $proxy) {
						$rows_affected = DB::update('update user_insta_profile set proxy = ? where id = ?;', [ $proxy->proxy, $ig_profile->id ]);
						$instagram->setProxy($proxy->proxy);
						$rows_affected = DB::update('update proxy set assigned = 1 where proxy = ?;', [ $proxy->proxy ]);
					}
				} else {
				...
				foreach ($dm_jobs as $dm_job) {
					if ($dm_job->follow_up_order == 1 && $user->user_tier <= 3) {
						$rows_affected_msg = DB::update('update dm_job set fulfilled = 1, success_msg = ?, updated_at = NOW() where job_id = ?;', [ "Not a Business user.", $dm_job->job_id ]);
						continue;
					}
					...
					try {
						...
						if ($response->status == "ok") {
							...
							$delay             = rand(30, 40);
							$rows_affected     = DB::update('update user_insta_profile set last_sent_dm = NOW() + INTERVAL ' . $delay . ' MINUTE where id = ?;', [ $ig_profile->id ]);
							$rows_affected_msg = DB::update('update dm_job set fulfilled = 1, success_msg = ?, updated_at = NOW() where job_id = ?;', [ serialize($response), $dm_job->job_id ]);

							//make sure follow up sends a day later.
							if ($ig_profile->auto_dm_delay == 1) {
								DB::update("UPDATE dm_job SET time_to_send = NOW() + INTERVAL 1 DAY "
						...
						$rows_affected_msg = DB::update('update dm_job set error_msg = ?, updated_at = NOW() where job_id = ?;', [ $checkpoint_ex->getMessage(), $dm_job->job_id ]);
					} catch (\InstagramAPI\Exception\NetworkException $network_ex) {
						$this->line($network_ex->getMessage());
						$rows_affected_msg = DB::update('update dm_job set error_msg = ?, updated_at = NOW() where job_id = ?;', [ $network_ex->getMessage(), $dm_job->job_id ]);
					} catch (\InstagramAPI\Exception\EndpointException $endpoint_ex) {
						$this->line($endpoint_ex->getMessage());
						$rows_affected_msg = DB::update('update dm_job set error_msg = ?, updated_at = NOW() where job_id = ?;', [ $endpoint_ex->getMessage(), $dm_job->job_id ]);
					} catch (\InstagramAPI\Exception\FeedbackRequiredException $feedbackrequired_ex) {
						$this->line($feedbackrequired_ex->getMessage());
						$rows_affected     = DB::update("UPDATE user_insta_profile SET last_sent_dm = NOW() + INTERVAL 1 DAY, temporary_ban = NOW() + INTERVAL 4 HOUR WHERE insta_username = ?;", [ $ig_profile->insta_username ]);
						$rows_affected_msg = DB::update('update dm_job set error_msg = ?, updated_at = NOW() where job_id = ?;', [ $feedbackrequired_ex->getMessage(), $dm_job->job_id ]);
					}
				}
			}
		}
		DB::delete("DELETE FROM morfix_settings WHERE setting = 'dm_offset' AND value = ?;", [ $offset ]);
	}
