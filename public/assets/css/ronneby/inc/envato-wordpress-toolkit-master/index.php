...
if (!class_exists('Envato_WP_Toolkit')) {
	...
	class Envato_WP_Toolkit {
		...
		protected function _constants() {
			 ...
			 */
			define('EWPT_PLUGIN_DIR', get_template_directory_uri());

			/**
			 * Plugin Directory URL
			 */
			define('EWPT_PLUGIN_URL', get_template_directory_uri());
		...
		protected function _includes() {
			...
			if (!class_exists('Envato_Theme_Upgrader')) {
				require_once locate_template('/inc/envato-wordpress-toolkit-master/includes/class-wp-upgrader.php');
			}
			if (!class_exists('Envato_Backup')) {
				require_once locate_template('/inc/envato-wordpress-toolkit-master/includes/class-envato-backup.php');
			}
			if (!class_exists('Envato_Protected_API')) {
				require_once locate_template('/inc/envato-wordpress-toolkit-master/includes/class-envato-api.php');
			}
			$options = get_option(EWPT_PLUGIN_SLUG);
			if (!class_exists('WP_GitHub_Updater') && !isset($options['deactivate_github_updater'])) {
				require_once locate_template('/inc/envato-wordpress-toolkit-master/includes/class-github-updater.php');
			}
		}
      ...
      <style>
        @font-face {
          font-family: "envato";
          src:url("' . get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/fonts/envato.eot?20141121");
          src:url("' . get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/fonts/envato.eot?#iefix20141121") format("embedded-opentype"),
            url("' . get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/fonts/envato.woff?20141121") format("woff"),
            url("' . get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/fonts/envato.ttf?20141121") format("truetype"),
            url("' . get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/fonts/envato.svg?20141121#envato") format("svg");
          font-weight: normal;
          font-style: normal;
        }
        #adminmenu .toplevel_page_envato-wordpress-toolkit .menu-icon-generic div.wp-menu-image:before {
      ...
      ';
		...
		public function _envato_load_scripts() {
			...
			wp_enqueue_script('ajax-notification', get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/js/ajax-notification.js', false, EWPT_PLUGIN_VER);
		}
		...
		public function _envato_load_styles() {
			wp_enqueue_style('envato-wp-updater', get_template_directory_uri() . '/inc/envato-wordpress-toolkit-master/assets/css/style.css', false, EWPT_PLUGIN_VER, 'all');
		}
					...
					if (!empty($themes) && is_array($themes)) {
						foreach ($themes as $theme) {
							...
							if (!empty($item_details)) {
								...
								$preview_url = htmlspecialchars(add_query_arg(array('preview' => 1, 'template' => $template, 'stylesheet' => $stylesheet, 'preview_iframe' => 1, 'TB_iframe' => 'true'), trailingslashit(esc_url(home_url()))));
								...
								if ($stylesheet && $template && $current_stylesheet !== $stylesheet) {
									...
									$links[] = '<a href="' . $delete_url . '" class="submitdelete deletion" title="' . esc_attr(sprintf(__('Delete &#8220;%s&#8221;', 'envato-wordpress-toolkit'), $title)) . '" ' . $delete_onclick . '>' . __('Delete', 'dfd') . '</a> |';
								...
								/* Tags list */
								if ($tags) {
									$content.= '<p>' . __('Tags: ', 'dfd') . join(', ', $tags) . '</p>';
								}
		...
		/* Custom code by DFD / Admin theme activation notice
		 * 
		 */
		function dfd_envato_toolkit_credentials_admin_notices() {
			$theme_name = wp_get_theme()->Name;

			$message = sprintf(
					__("To enable theme update notifications, please enter your Envato Marketplace credentials in the %s", "default"),
					"<a href='" . admin_url() . "admin.php?page=envato-wordpress-toolkit'>'{$theme_name}' Update Options</a>"
				);
			echo "<div id='message' class='updated notice below-h2 is-dismissible'><p>{$message}</p></div>";
		}
		
		function dfd_admin_notice_enable() {
			$options = get_option(EWPT_PLUGIN_SLUG);
			...
			if(empty($user_name) || empty($api_key)) {
				add_action('admin_notices', array($this, 'dfd_envato_toolkit_credentials_admin_notices'));
				return false;
			}
			return true;
		}
