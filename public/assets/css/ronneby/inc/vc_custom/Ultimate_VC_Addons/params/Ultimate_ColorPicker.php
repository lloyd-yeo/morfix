...
if(!class_exists('Ultimate_ColorPicker_Param'))
{
	class Ultimate_ColorPicker_Param
	{
		...
		function colorpicker_alpha_gen($settings, $value)
		{
			...
			$output = '
                <input id="alpha_val'.$uni.'" class="wpb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . ' vc_column_alpha" value="'.$value.'" data-uniqid="'.$uni.'" data-opacity="'.$opacity.'" data-hex-code="'.$base.'"/>
';
			$output .= '
<input class="wpb_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="'.$param_name.'" value="'.$value.'" style="display:none"/>
