...
if(!class_exists('Ult_Spacing'))
{
  class Ult_Spacing
  {
    function __construct()
    {
		...
		{
			  vc_add_shortcode_param('ultimate_spacing', array($this, 'ultimate_spacing_callback'), get_template_directory_uri().'/inc/vc_custom/Ultimate_VC_Addons/admin/vc_extend/js/ultimate-spacing.js');
		}
    }
    ...
    function ultimate_spacing_callback($settings, $value)
    {
        ...
        $html .= '  <input type="hidden" data-unit="'.$unit.'" name="'.$settings['param_name'].'" class="wpb_vc_param_value ultimate-spacing-value '.$settings['param_name'].' '.$settings['type'].'_field" value="'.$value.'" />';
        $html .= '</div>';
      return $html;
    }
    ...
    function ultimate_spacing_param_scripts($hook) {
		...
		wp_register_style( 'ultimate_spacing_css', get_template_directory_uri().'/inc/vc_custom/Ultimate_VC_Addons/admin/vc_extend/css/ultimate_spacing.css');
