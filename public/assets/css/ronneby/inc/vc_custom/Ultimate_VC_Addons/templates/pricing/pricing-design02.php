	...
	class Pricing_Design02 extends Ultimate_Pricing_Table{
		public static function generate_design($atts,$content = null){
			...
			$output .= '<div class="dfd-pricing-cover '.esc_attr($animate).'" '.$animation_data.'>
						<div class="ult_pricing_table_wrap ult_design_2 '.esc_attr($featured).' '.esc_attr($hot).' '.esc_attr($el_class).'">
					...
					$output .= '<div class="bottom-part">';
						...
						if($package_btn_text !== ""){
							$output .= '<div class="ult_price_link" style="'.$normal_style.'">
											<a href="'.esc_url($link).'" '.$target.' class="ult_price_action_button button" style="'.$featured_style.' '.$button_inline.'">'.$package_btn_text.'</a>
										</div><!--ult_price_link-->';
						}
