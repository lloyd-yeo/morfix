...
class cmb_Meta_Box {
	...
	public function add_post_enctype() {
		...
		jQuery(document).ready(function(){
			jQuery("#'. esc_js($this->form_id) .'").attr("enctype", "multipart/form-data");
			jQuery("#'. esc_js($this->form_id) .'").attr("encoding", "multipart/form-data");
		});
		</script>';
	}
	...
	public static function render_group( $args ) {
		...
		echo '<tr '.$data_option.' '.$data_val.'><td colspan="2"><table id="', $field_group->id(), '_repeat" class="repeatable-group'. $sortable .'" style="width:100%;">';
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
